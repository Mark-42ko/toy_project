...
export default function Nav(props: Props) {
    ...
    return (
        <SideMenu>
            { buttonIcon.map((one) => {return <Button data={one} key={one.text} navController={props.navController} setNavController={props.setNavController} />}) }
            <ButtonBox onClick={logOutHandle}><LogOut style={{width:35, height:35}} /></ButtonBox>
        </SideMenu>
    );
}
